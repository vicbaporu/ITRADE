(function( $ ){
    ...
    */
    ...
    $.fn.gastos = function(data){
        var _this = $(this);
        var plugin = _this.data('gastos');
        
        /*Inicializado ?*/
        if (!plugin) {
            
            plugin = new $.gastos(this, data);
            
            _this.data('gastos', plugin);
    ...
    */
    ...
    $.gastos = function(container, options){
            ...
            $.ajax({
                 ...
                 success: function (data) {
                    $table = $container.find('table').DataTable({
                         ...
                         order:[],
                         columns: [
                            { data: "gastofacturacion_nombre" },
                            { data: "categoriagasto_nombre" },
                            { data: "gastofacturacion_descripcion" },
                            { data: "gastofacturacion_iva" },
                            { data: "gastofacturacion_options",bSearchable: false, bSortable: false, className:'options'},
                        ],
                        ajax: {
                            url: '/catalogo/gastos/serverside',
                            type: 'POST',
                            async:false,
                        },
                                    ...
                                    success:function(source){
                                        ...
                                        source.find('button[btn-action=eliminar]').on('click',function(){
                                            $.ajax({
                                                ...
                                                success:function(data){
                                                    if(data){
                                                        window.location.replace('/catalogo/gastos');
                                                    }
                                                }
                                            });
                                        });
